...
dpg.create_context()
dpg.create_viewport(title="Cloudy Music",large_icon="icon.ico",small_icon="icon.ico")
pygame.mixer.init()
...
with dpg.font_registry():
	monobold = dpg.add_font("fonts/Cascadia.ttf", 14)
	head = dpg.add_font("fonts/Cascadia.ttf", 16)
...
with dpg.window(tag="main",label="window title"):
	...
	with dpg.group(horizontal=True):
		with dpg.child_window(width=200,tag="sidebar"):
			...
			dpg.add_text("Build by Kishore DT")
			dpg.add_spacer(height=2)
			dpg.add_button(label="Support",width=-1,height=23,callback=lambda:webbrowser.open(url="https://github.com/KishoreDT/Music-Player"))
